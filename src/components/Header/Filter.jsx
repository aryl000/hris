...
const month = parseInt(new Date().getMonth(), 10)
...
class Filter extends Component {
    ...
    change(e) {
        ...
        //         month: "January"
        //     });
        //     this.state.filters["month"] = "January"
        // }
        this.setState({
    ...
    submit(e) {
        e.preventDefault();
        let filter = []
        if (['/Rejected_Candidates', '/dashboard'].indexOf(window.location.pathname) !== -1) {
            // filter = {
            //     year: this.state.years,
            //     type: this.state.type
            // }
            if (this.state.daily === 'week' || this.state.daily === 'day')
                filter.push('type=' + this.state.type)
            else
                filter.push('year=' + this.state.years, 'type=' + this.state.type)
        }
        else {
            // filter = {
            //     year: this.state.years,
            // }
            if (this.state.daily !== 'week' && this.state.daily !== 'day')
                filter.push('year=' + this.state.years)
        }

        if (this.state.daily === "quarter") {
            filter.push('quarter=' + this.state.quarter, 'daily=' + this.state.daily)
            // filter["quarter"] = this.state.quarter;
            // filter["daily"] = this.state.daily;
        }
        else if (this.state.daily === "month") {
            filter.push('month=' + this.state.month, 'daily=' + this.state.daily)
            // filter["month"] = this.state.month;
            // filter["daily"] = this.state.daily;
        }
        else if (this.state.daily !== "year") {
            filter.push('daily=' + this.state.daily)
            // filter = {
        ...
        this.props.history.push(`${url}?${filter}`)
        filter = []
        //daily untuk di quarter dan month sepertinya ngk perlu
    ...
    monthly() {
        ...
        let check = null;
        if (this.state.years !== "all" || this.state.year === year.toString()) {
            for (let i = 1; i <= (month + 1); i++) {
                arr.push(<option key={i} value={i}>{monthly[i - 1]}</option>)
            }
        } else {
            monthly.map((el, index) =>
                arr.push(<option key={el} value={(index + 1)}>{el}</option>)
        ...
        return (
            <NavItem eventKey={3} id="navitem">
                <OverlayTrigger trigger={['hover', 'focus']} placement="left" overlay={popoverType}>
                    <select name="type" value={this.state.type} id="navbarType" onChange={e => this.change(e)}>
        ...
        return (
            <NavItem eventKey={4} id="navitem">
                <OverlayTrigger trigger={['hover', 'focus']} placement="left" overlay={
                    ...
                    <select name="daily" value={this.state.daily} id="navbarType" onChange={e => this.change(e)} >
            ...
            <NavItem eventKey={2} id="navitem">
                <div style={{ display: ['week', 'day'].indexOf(this.state.daily) !== -1 ? "none" : "block" }}>
                    <OverlayTrigger trigger={['hover', 'focus']} placement="left" overlay={popoverYear}>
                        <select name="years" value={this.state.years} id="navbarYear" onChange={e => this.change(e)}>
                            <option value="all">All</option>
                            {this.yearly()}
                        </select>
                    </OverlayTrigger>
            ...
            <NavItem eventKey={5} id="navItemQ">
                <div style={{ display: this.state.daily === "quarter" ? "block" : "none" }}>
                    <OverlayTrigger trigger={['hover', 'focus']} placement="left" overlay={popoverQuarter}>
                        <select name="quarter" value={this.state.quarter} id="navbarQ" onChange={e => this.change(e)}>
                            {this.quarterly()}
                        </select>
                    </OverlayTrigger>
                </div>
        ...
        return (
            <NavItem eventKey={5} id="navItemM">
                <div style={{ display: this.state.daily === "month" ? "block" : "none" }}>
                    <OverlayTrigger trigger={['hover', 'focus']} placement="left" overlay={popoverQuarter}>
                        <select name="month" value={this.state.month} id="navbarM" onChange={e => this.change(e)}>
                            {this.monthly()}
                        </select>
                    </OverlayTrigger>
                </div>
            ...
            <div className="filter">
                ...
                <div className="filter-container">
                    ...
                    <div style={{ display: this.state.quarter !== "all" ? this.state.daily !== "quarter" ? "none" : "block" : "none" }}>
                        ...
                        >
                            <Link to={{ pathname: '/user/bob', query: { showAge: true } }}>
